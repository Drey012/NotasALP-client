...
function NumberField({ label, value, onChange, disabled = false }: { label: string; value: string; onChange: (value: string) => void; disabled?: boolean }) {
  ...
  return <label className="field"><span>{label}</span><input inputMode="decimal" type="number" min="0" max="10" step="0.1" value={value} onChange={(event) => handleChange(event.target.value)} placeholder="0,0" disabled={disabled} /></label>;
}
...
export default function Home() {
  ...
  const filledGrades = useMemo(() => grades.filter((grade) => grade.trim() !== ""), [grades]);
  const canEvaluate = useMemo(() => filledGrades.length > 0 && filledGrades.every((grade) => Number(grade) >= 0 && Number(grade) <= 10), [filledGrades]);
  ...
  const submit = async () => {
    if (!canEvaluate) { setError("Preencha ao menos uma nota entre 0 e 10."); return; }
    setLoading(true); setError("");
    try {
      setResult(await evaluateNotes({
        indiceProfessor: professorIndex,
        notasIniciais: filledGrades.map(Number),
        p3: p3 ? Number(p3) : undefined,
        exame: exam ? Number(exam) : undefined
      }));
